...
export default function ShowsRender({Data}){
            ...
            <button className={Styles.ScrollLeft} onClick={handelScrollLeft}>&nbsp;&nbsp; &#x276E;&#x276E; &nbsp;&nbsp;&nbsp;&nbsp;</button>
                {Data[0].map((item) => (
                    <div key={item.ID} className={Styles.OneImage}>
                        <Link href={`/entertainment/player/${item.ID}/${item.Title.split(" ").join("-")}`}><Image className={Styles.Images} src={item.CoverImage} alt={`Image ${item.ID}`} title={item.Title} width={300} height={300} /></Link>
                    </div>
                ))}
