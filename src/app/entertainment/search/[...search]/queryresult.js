...
export default async function SearchResult(query){
    ...
    const res = await supabase.from(process.env.NEXT_PUBLIC_SUPABASE_MOVIES_DATABSE_NAME).select('ID,Title,Image').order('ID', { ascending: false }).ilike('Title', `%${search}%`).range(0,15);
    return res.data;
}